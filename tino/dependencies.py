...
def require_global_admin(user: User = Depends(get_current_user)) -> User:
    '''Require the user to be a global TINO administrator.'''
...
def get_search_service() -> SearchService:
    '''Singleton SearchService composing the shared FileService.'''
    return SearchService(get_file_service())


@lru_cache
def get_compiler_service() -> CompilerService:
    '''Singleton CompilerService bound to the configured bucket directory.'''
    return CompilerService(config.TINO_BUCKET_DIR, config.TINO_PACKAGE_DIR,
                           config.TINO_FONT_DIR)


@lru_cache
def get_template_service() -> TemplateService:
    '''Singleton TemplateService bound to the configured bucket directory.'''
    return TemplateService(config.TINO_BUCKET_DIR, config.TINO_PACKAGE_DIR)


@lru_cache
def get_font_service() -> FontService:
    '''Singleton FontService bound to the configured font directory.'''
    return FontService(config.TINO_FONT_DIR)
...
def get_collab_manager() -> CollabManager:
    ...
    return CollabManager(
        data_dir=config.TINO_BUCKET_DIR,
        file_service=get_file_service(),
        auto_save=config.TINO_SAVE_DEBOUNCE_MS > 0,
        room_ttl=config.TINO_ROOM_TTL,
    )
