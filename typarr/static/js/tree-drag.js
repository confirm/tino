...
export class TreeDrag {
  ...
  bindUploadDrop() {
    ...
    zone.addEventListener('dragover', evt => {
      if (evt.dataTransfer.types.includes('text/x-typarr-path'))
        return
      evt.preventDefault()
      if (this._canEdit())
  ...
  static _onDragStart(evt) {
    ...
    evt.dataTransfer.setData('text/x-typarr-path', path)
    evt.dataTransfer.setData('text/x-typarr-type', type)
  ...
  async _onTreeDrop(evt) {
    ...
    evt.stopPropagation()
    this._clearDropTargets()
    const srcPath = evt.dataTransfer.getData('text/x-typarr-path')
    const srcType = evt.dataTransfer.getData('text/x-typarr-type')
